        ...
        while(System.currentTimeMillis() - startTime < executeTime) {
            try {
                ...
                if(pedestriansNo == maxPedestriansNo) {
                    pedestriansNo = 0;
                    pass = true;
                    // notify that light gas beed changed
                    synchronized ((Crosswalk) Main.intersection) {
                        ((Crosswalk) Main.intersection).light = !((Crosswalk) Main.intersection).light;
                        Main.intersection.notifyAll();
                    }
                    sleep(Constants.PEDESTRIAN_PASSING_TIME);
                    pass = false;
                    // notify that light has been changed
                    synchronized ((Crosswalk) Main.intersection) {
                        ((Crosswalk) Main.intersection).light = !((Crosswalk) Main.intersection).light;
                        Main.intersection.notifyAll();
                    }
